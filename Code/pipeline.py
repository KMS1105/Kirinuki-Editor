...
class CutEngine:
    ...
    def process_video(self, input_path, preset_name):
        ...
        if raw_preset_data and isinstance(raw_preset_data, dict) and "data" in raw_preset_data:
            preset_data = raw_preset_data["data"].get("cut", raw_preset_data)
            print(f"[LOG] 프리셋에서 'data' -> 'cut' 설정을 성공적으로 가져왔습니다.")
        else:
